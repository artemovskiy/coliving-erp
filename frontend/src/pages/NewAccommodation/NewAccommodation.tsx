import { CircularProgress, Divider, FormControl, Grid, InputLabel, MenuItem, Paper, Select, Typography } from '@mui/material';
import { ResidentSelect } from '../../components/common/ResidentSelect';
import { useApi } from '../../providers/ApiClient';
import { useApiFetch } from '../../api/useApiFetch';
import { CreateAccommodation, House, Resident, Room, Slot } from 'coliving-erp-api-client';
...
function NewAccommodation() {
  const { residentsApi, housesApi, roomsApi, slotsApi, accommodationsApi } = useApi();

  const [ residents, residentsPending, residentsError ] = useApiFetch<Resident[], []>(() => residentsApi.listResidents(), []);
  ...
  const [ rooms, roomsPending ] = useApiFetch<Room[] | undefined, [number | undefined]>((houseId: number | undefined) => {
  ...
  const [ slots, slotsPending ] = useApiFetch<Slot[] | undefined, [number | undefined]>((roomId: number | undefined) => {
    if(roomId === undefined) {
      return Promise.resolve({ data: [] } as unknown as AxiosResponse<Room[]>);
    }
  ...
  const isSubmitAvailable = useMemo(() => {
    return !!residentId && !!houseId && !!roomId && !!slotId && (daysLength ?? 0) > 0;
          ...
          <Paper sx={{ padding: 2}}>
            ...
            <Grid container direction='column' spacing={1}>
              <Grid item>
              {
                ...
                >
                  {
                    houses.map(house =>  <MenuItem value={house.id!} key={house.id}>#{house.id} {house.name}</MenuItem>)
                  }
                </Select>
              </FormControl>
              }
              ...
              <Grid item>
                <FormControl fullWidth>
                  ...
                  >
                    {
                      roomOptions.map(room =>  <MenuItem value={room.id!} key={room.id}>#{room.id} {room.name}</MenuItem>)
                    }
                  </Select>
                </FormControl>
              </Grid>
              <Grid item>
                <FormControl fullWidth>
                  ...
                  >
                    {
                      slotOptions.map(slot =>  <MenuItem value={slot.id!} key={slot.id}>#{slot.id} {slot.label}</MenuItem>)
                    }
                  </Select>
                </FormControl>
              </Grid>
        ...
        <Grid item>
          ...
          <Paper sx={{ padding: 2}}>
            <Grid container spacing={2}>
              ...
              <Grid item lg={12}>
                { daysLength !== undefined && <Typography variant='body1'>
                  Days: {daysLength}
                </Typography> }
              </Grid>
              <Grid item lg={12}>
                { monthsLength !== undefined && <Typography variant='body1'>
                  Months: {monthsLength}
                </Typography> }
              </Grid>
            </Grid>
