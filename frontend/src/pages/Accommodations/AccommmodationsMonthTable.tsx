import { Interval, differenceInCalendarDays, differenceInDays, interval } from 'date-fns';
import { AccommodationData, AccommodationsSheet } from './types';
import { Box, IconButton, Paper, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Toolbar, Tooltip, Typography } from '@mui/material';
        ...
        <Table size='small'>
          <TableHead>
            <TableRow>
              ...
              {
                data.months.map(i => <TableCell sx={{borderRight: 1}} key={i.isoFirstDay} colSpan={i.length}>{i.name}</TableCell>)
              }
            </TableRow>
            <TableRow>
              {
                data.dates.map(i => <TableCell sx={{borderRight: 1}} key={i.toISOString()}>{i.toLocaleString('default', { day: '2-digit'})}</TableCell>)
              }
            </TableRow>
          </TableHead>
          <TableBody>
            { 
              data.slots.map(slot => {
                const accommodationCells = prepareAccommodationCells(interval.start as Date, interval.end as Date,slot.accommodations);
                return <TableRow key={slot.name}>
                  <TableCell scope='row'  sx={{borderRight: 1}}>{slot.name}</TableCell>
                  {
                    accommodationCells.map(i => {
                      if(i.isEmpty) {
                        return <TableCell key={i.label} colSpan={i.length} aria-labelledby='empty'></TableCell>
                      }
...
//         {
//           data.months.map(i => <th key={i.isoFirstDay} colSpan={i.length}>{i.name}</th>)
//         }
//       </tr>
//       <tr>
//         {
//           data.dates.map(i => <th key={i.toISOString()}>{i.toLocaleString('default', { day: '2-digit'})}</th>)
//         }
//       </tr>
//     </thead>
//     <tbody>
//       { 
//         data.slots.map(slot => {
//           const accommmodationCells = prepareAccommodationCells(interval.start as Date, interval.end as Date,slot.accommodations);
//           return <tr key={slot.name}>
//             <th scope='row'>{slot.name}</th>
//             {
//               accommmodationCells.map(i => <td key={i.label} colSpan={i.length}>{i.label}</td>)
//             }
//           </tr>
//         })  
//       }
